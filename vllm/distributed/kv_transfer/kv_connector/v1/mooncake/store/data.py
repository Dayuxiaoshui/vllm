...
class BoundaryStoreStats:
    """Cumulative counters for mamba boundary-state hand-offs.

    ``store_mask`` excludes mamba groups from the positional save, so a
    hand-off is the only way a mamba state is ever persisted, and every branch
    that declines one is silent: the state is simply not written and a later
    request misses on that group. The only symptom is a lower hit rate with
    nothing in the logs, so each decline reason is counted here and exposed
    through ``MooncakeStoreConnector.get_boundary_store_stats()``.

    Counters are in hand-off entries as offered by the core, not in store
    keys: rank sharding drops a key on every rank but one, and a sub-block
    tail entry contributes keys for every group. Never cleared.
    """
